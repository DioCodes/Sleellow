...
export const Star = ({ height, paused }) => {
  ...
  const randomStarSize = useRef(
    new Animated.Value(randomIntFromInterval(0.5, starMaxSize))
  ).current;
  ...
  let loopStarAnimation = () => {
    ...
    Animated.parallel([
      ...
      Animated.timing(valXY.y, {
        toValue: randomIntFromInterval(0, parentHeight) * -1,
        duration: time,
        useNativeDriver: true,
        easing,
      }),
...
const styles = StyleSheet.create({
  ...
  starPartFirst: {
    transform: [{ scaleX: 1.25 }, { scaleY: 0.25 }],
  },
  starPartSec: {
    transform: [{ scaleX: 0.25 }, { scaleY: 1.25 }],
  },
});
